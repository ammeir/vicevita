...
{
	m_state = SAVESLOTS_INITIAL;

	if (g_game_file.empty())
		return;
	
	m_state = SAVESLOTS_INGAME_NOSAVES;
...
int SaveSlots::applyPatchModuleSettings(const char* snapshot)
{
	...
	// We do this last because fillMappingValuesBuf() also uses strtok.
	if (!keymaps.empty()){
		m_controls->fillMappingValuesBuf(keymaps.c_str());
	}
